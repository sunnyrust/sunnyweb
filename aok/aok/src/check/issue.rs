...
impl Display for Diff {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        if self.type_changed {
            ...
            f.write_str(" aok_type: ")?;
            f.write_str(&self.aok_type)?;
        }
        if self.db_nullable != self.aok_nullable {
            f.write_str(" db_null: ")?;
            Display::fmt(&self.db_nullable, f)?;
            f.write_str(" aok_null: ")?;
            Display::fmt(&self.aok_nullable, f)?;
        }
        Ok(())
    }
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
/// Describes a potential problem or different between the aok struct and the database table
pub struct Issue {
    ...
    pub(crate) fn changed(
        ...
        Issue {
            ...
            kind: Kind::Changed(Diff {
                ...
                db_nullable: db.null,
                aok_type: st.dbtype().to_string(),
                aok_nullable: st.nullable(),
            }),
        }
    }
}
...
pub enum Kind {
    ...
    MissingTable,
    /// The column is defined in the database but not the aok Struct
    InDbNotModel(Missing),
    /// The column is defined on the aok Struct but not in the database
    OnModelNotDb(Missing),
    /// How the model is defined in the database table is different than on the aok Struct
    Changed(Diff),
}
