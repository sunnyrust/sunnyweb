...
impl<'a> NavigationNode<'a> {
    ...
    pub fn add_child(&mut self, child: &'a RefCell<NavigationNode<'a>>) {
...
#[allow(dead_code)]
pub async fn get_all_tag<'a,'b>(state: &'a AppState,order:Option<&'b str>) -> Result<Vec<Model>> {
...
#[allow(dead_code)]
pub async fn get_navigation_tree<'a>(state: &'a AppState) -> Result<Vec<NavigationModel>> {
    // 操作redis
    ...
    let rv:redis::RedisResult<String> = redis_conn.get("navigation_tree");//读取缓存
    let result =match rv {
        Ok(issue) => {
            b_have_key=true;
            issue
    ...
    if !b_have_key{
        ...
        for parent in tree.iter() {
            ...
            parent.borrow_mut().set_node_to_model(0, &mut navigation_models);
        }

        // 插入redis
        let strm:String=serde_json::to_string(&navigation_models).unwrap();
        let _:()=redis_conn.set("navigation_tree",  strm).unwrap();

        

    }else{
        //把从redis里面取出来的字符串，反序列化为NavigationModel结构
        navigation_models=serde_json::from_str(&result).unwrap();
    }
    

    Ok(navigation_models)
}


#[allow(dead_code)]
pub async fn delete<'a>(state: &'a AppState,id:i32) -> Result<String> {
    let pool = get_db_conn(&state);
    ...
    let client=get_redis_conn(&state);
    let mut redis_conn = client.get_connection().expect("redis connect error");
    let _=redis::cmd("DEL").arg("navigation_tree").exec(&mut redis_conn);
    Ok("ok".to_string())
}

/// 判断id是不是一个pid
async fn is_parent<'a>(state: &'a AppState,id:i32) ->Result<bool>{
   let pool = get_db_conn(&state);
...
/// 判断名字是不是在数据库中存在
async fn have_name<'a>(state: &'a AppState,name:String) ->Result<bool>{
    let pool = get_db_conn(&state);
...
#[allow(dead_code)]
pub async fn insert_one<'a>(state: &'a AppState,m:Model) -> Result<String> {
    let pool = get_db_conn(&state);
